...
%% Get sensor data
% Select source of sensordata
if usingArduino
    % instrfind('Port','COM4') % Check if arduino is in use 
    arduino = serial('COM4','BaudRate',9600);
    arduino.Terminator = 'CR/LF';
    fopen(arduino);
    
    % Send start command and wait for incoming data
    fwrite(arduino,'A');
    pause(2);
    sensorData = fgetl(arduino);
...
if usingWebcam
    ...
    % Get demo image
    img = imread('Demo Images/marker_demo.png');
end
